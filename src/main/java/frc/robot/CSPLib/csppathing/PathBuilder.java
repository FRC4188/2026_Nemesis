...
public final class PathBuilder {
  ...
  private static final double RECOVERY_ERROR_THRESHOLD_METERS = 0.60;
  private static final double RECOVERY_LOOKAHEAD_METERS = 0.25;
  private static final double RECOVERY_SAMPLE_STEP_METERS = 0.10;

  private static boolean recoveryTriggered = false;
  private static double recoveryStartArc = 0.0;
  ...
  private static double fieldLengthMeters = 0;
  ...
  public static void configureField(double width, double length) {
  ...
  public static PathPlannerPath build(Target... targets) {
    if (targets == null || targets.length == 0) {
      throw new IllegalArgumentException("Must supply at least one Target");
    }

    final double rotIndexRadius = PATH_CREATION_TOL;
  ...
  public static Command path(Target... targets) {
    ...
    Command followCmd = followWithRecovery(finalPath);
  ...
  private static Command followWithRecovery(PathPlannerPath finalPath) {
    recoveryTriggered = false;
    recoveryStartArc = 0.0;

    Command normalFollow = AutoBuilder.followPath(finalPath);
    ...
    Command monitor =
        Commands.run(
                () -> {
                  ...
                  double arc =
                      getClosestArc(pose.getTranslation(), activeFollowSampled, activeFollowCum);
                  Pose2d closest = samplePoseAtArc(arc);

                  double errorMeters = pose.getTranslation().getDistance(closest.getTranslation());

                  if (errorMeters > RECOVERY_ERROR_THRESHOLD_METERS) {
                    recoveryTriggered = true;
                    recoveryStartArc =
                        Math.min(arc + RECOVERY_LOOKAHEAD_METERS, activeFollowTotalLength);
                  }
                })
            .until(() -> recoveryTriggered);

    Command recovery =
        Commands.defer(
            () -> AutoBuilder.followPath(buildRecoverySuffixPath(recoveryStartArc)),
            Set.of(driveSubsystem));

    return Commands.race(normalFollow, monitor)
        .andThen(Commands.either(recovery, Commands.none(), () -> recoveryTriggered));
  }

  private static Pose2d samplePoseAtArc(double arc) {
    Translation2d translation = sampleAtArc(activeFollowSampled, activeFollowCum, arc);
    Rotation2d rotation = getExplicitRotationAtArc(arc);
    return new Pose2d(translation, rotation);
  }

  private static PathPlannerPath buildRecoverySuffixPath(double startArc) {
    List<Pose2d> suffixPoses = new ArrayList<>();

    double arc = Math.max(0.0, Math.min(startArc, activeFollowTotalLength));
    while (arc < activeFollowTotalLength) {
      suffixPoses.add(samplePoseAtArc(arc));
      arc += RECOVERY_SAMPLE_STEP_METERS;
    }
    suffixPoses.add(samplePoseAtArc(activeFollowTotalLength));

    List<Waypoint> waypoints = posesToWaypoints(suffixPoses);

    GoalEndState goal =
        new GoalEndState(0.0, suffixPoses.get(suffixPoses.size() - 1).getRotation());
  ...
  private static List<Waypoint> posesToWaypoints(List<Pose2d> poses) {
    ...
    for (int i = 0; i < poses.size(); i++) {
      ...
      if (poses.size() == 1) {
        ...
        Translation2d next = poses.get(i + 1).getTranslation();
        heading =
            safeHeading(
                next.getX() - anchor.getX(), next.getY() - anchor.getY(), pose.getRotation());
      } else {
        Translation2d prev = poses.get(i - 1).getTranslation();
        heading =
            safeHeading(
                anchor.getX() - prev.getX(), anchor.getY() - prev.getY(), pose.getRotation());
      }

      Translation2d dir = new Translation2d(heading.getCos(), heading.getSin());

      double prevDist = 0.0;
      double nextDist = 0.0;

      if (i > 0) {
        prevDist = Math.min(anchor.getDistance(poses.get(i - 1).getTranslation()) * 0.33, 0.35);
      }
      if (i < poses.size() - 1) {
        nextDist = Math.min(anchor.getDistance(poses.get(i + 1).getTranslation()) * 0.33, 0.35);
      }

      Translation2d prevControl = anchor.minus(dir.times(prevDist));
      Translation2d nextControl = anchor.plus(dir.times(nextDist));
  ...
  public static Command triggerWhenClose(
    ...
    return Commands.waitUntil(
            () -> getPose.get().getTranslation().getDistance(apply(location)) <= distance)
        .andThen(runnable);
  }
  ...
  public static Command createPath(Pose2d endPose) {
    return AutoBuilder.pathfindToPose(endPose, constraints, 0.0).finallyDo(stopDrive);
  }
